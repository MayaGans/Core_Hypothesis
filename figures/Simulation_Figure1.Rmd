...
sim.df_all <- sim.df %>%
  ...
  mutate(method =
    case_when(
      ...
      method == "PropRead" ~ "Proportion of Sequence Reads",
      method == "PropReps" ~ "Proportion of Sequence Replicates",
      method == "PropReadReps" ~ "Proportion of Sequence Reads and Replicates"
    ) %>%
      factor(levels = c("Proportion of Sequence Reads", "Proportion of Replicates", "Proportion of Sequence Reads and Replicates", "Hard Cutoff"))
  )
```
...
```{r}
g <- (tp_plot + fp_plot + tf_plot + plot_layout(ncol=1))
ggsave(file="figure1.pdf", g, width = 25, height = 15) #saves g
```