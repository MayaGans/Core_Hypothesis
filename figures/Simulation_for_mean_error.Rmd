...
sim.df_gg <- sim.df %>%
  ...
  mutate(method = factor(method, levels = c("meanerrorHardCut",
                                            "meanerrorPropRead",
                                            "meanerrorPropReps", "meanerrorPropReadReps"), labels = c("Hard Cutoff", "Proportion of Reads", "Proportion of Replicates", "Proportion of Reads and Replicates")))
...
ggplot(sim.df_gg, aes(x = as.factor(corenoncoreratio), y = as.factor(theta))) +
  ...
  theme_minimal() +
  scale_fill_gradientn(colours = myPalette, values = scales::rescale(c(-25, 0, 100, 500, 1000))) +
  geom_text(aes(label = round(prob, digits = 1))) +
